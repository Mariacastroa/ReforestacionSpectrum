...
def calcular_impacto_medio_ambiente(
    ...
    """Marcador temporal para la fórmula ambiental pendiente.

    Reemplaza el ``return 0.0`` cuando recibas la fórmula definitiva.
    Los parámetros ya están disponibles para utilizarse en el cálculo.
    """
    _ = (
...
def crear_panel_comparacion_superficies(
    ...
    impacto_medio_ambiente = calcular_impacto_medio_ambiente(
        impacto_neto_campo_m2=impacto_neto_campo_m2,
        impacto_plantado_m2=impacto_plantado_m2,
        impacto_removido_m2=impacto_removido_m2,
    )
...
def inicializar_estado() -> None:
    ...
    area_guardada = datos.get("area_general")
    if area_guardada in {"Ciudad de Guatemala", "Synergy"}:
...
def construir_mapa() -> folium.Map:
    ...
    if seleccion is not None:
        ...
        nombre_seleccion = st.session_state.ubicacion
        color_relleno = "#8BC34A" if es_ciudad else "#9CCC65"
        color_borde = "#0B5D3B" if es_ciudad else "#173B57"

        # Resalta con mayor intensidad la zona o sector escogido.
        folium.GeoJson(
            seleccion,
            name="Ubicación seleccionada",
            style_function=lambda _, relleno=color_relleno, borde=color_borde: {
                "fillColor": relleno,
                "color": borde,
                "weight": 5,
                "fillOpacity": 0.58,
            ...
            highlight_function=lambda _, relleno=color_relleno, borde=color_borde: {
                "fillColor": relleno,
                "color": borde,
                "weight": 6,
                "fillOpacity": 0.70,
            },
            tooltip=f"{nombre_seleccion} seleccionada",
        ).add_to(mapa)

        # Coloca una etiqueta permanente en el centro de la zona seleccionada.
        # Así, al elegir Zona 18, el mapa muestra claramente “Zona 18”.
        try:
            centro_seleccion = shape(
                seleccion["geometry"]
            ).representative_point()

            folium.Marker(
                location=[centro_seleccion.y, centro_seleccion.x],
                tooltip=f"{nombre_seleccion} seleccionada",
                z_index_offset=1000,
                icon=folium.DivIcon(
                    icon_size=(132, 36),
    ...
    if proyecto_edicion is None:
        ...
        with st.container(border=True):
            ...
            area_editada = st.radio(
                ...
                index=(
                    0
                    if proyecto_edicion["area_general"]
                    == "Ciudad de Guatemala"
                    else 1
                ),
                horizontal=True,
            ...
            opciones_edicion = (
                (ZONE_NAMES or ["Zonas no disponibles"])
                if area_editada == "Ciudad de Guatemala"
                else ORDEN_SECTORES_SYNERGY
            )
            ...
            ubicacion_editada = st.selectbox(
                "Zona" if area_editada == "Ciudad de Guatemala" else "Sector",
                opciones_edicion,
                index=(
